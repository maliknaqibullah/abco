<div>
    <div class="bg-white px-4 py-5 border-b border-gray-200 sm:px-6 mb-5">
        <div class="-ml-4 -mt-2 flex items-center justify-between flex-wrap sm:flex-nowrap">
            <div class="ml-4 mt-2">
                <h3 class="text-lg leading-6 font-medium text-gray-900"> Take School Attendance</h3>
            </div>
                        ...
                        @if(count($attendances)>0)
                            ...
                            @foreach ($attendances as $attendance)
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ $attendance->male ?$attendance->male :'No Male Students'    }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ $attendance->female ?$attendance->female :'No Female Students'}}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">

                                        <div
                                            class="text-sm text-gray-900 font-semibold uppercase">{{
                                            $attendance->school->district->province->name }}</div>
                                        <div class="text-sm text-gray-500 capitalize">
                                            {{$attendance->school->district->name}}
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <button wire:click="edit({{ $attendance->id }})" type="button"
                                        ...
                                        <button wire:click="deleteId({{ $attendance->id }})"
        ...
        <x-slot name="content">
            <div class="intro-y box p-5">
                <form wire:submit.prevent="saveSchool" class="pb-10 ">
                    ...
                    <div class="mt-3">
                        <x-jet-label for="selectedSchool" value="{{ __('Select School') }}"/>
                        <select wire:model="selectedSchool" id="selectedSchool" autocomplete="selectedSchool"
                                class="select-option"
                                required>
                            <option value="">-- Choose School --</option>
                            @foreach ($schools as $school)
                                <option value="{{ $school->id }}">{{ Str::title($school->name) }}</option>
                            @endforeach
                        </select>
                        <x-jet-input-error for="selectedSchool" class="mt-2"/>

                    </div>
                    <div class="mt-3">
                        <x-jet-label for="className" value="{{ __('Class Name') }}"/>
                        <x-jet-input id="className" type="text" class="mt-1 block w-full" wire:model.defer="className"
                                     autocomplete="className"/>
                        <x-jet-input-error for="className" class="mt-2"/>
                    </div>
